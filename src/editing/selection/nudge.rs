...
pub fn handle_nudge_input(
    ...
    if nudge_direction != Vec2::ZERO {
        ...
        if selected_count > 0 {
            ...
            // STEP 1.5: Handle smooth curve constraints for any off-curve points that were moved
            let mut smooth_adjustments = Vec::new();

            // Get the current glyph name from any point movement
            if let Some(first_movement) = all_movements.first() {
                ...
                let all_point_data: Vec<_> = queries.p2().iter()
                            ...
                            if movement.is_connected_offcurve {
                                if left_handle == Some(movement.entity) || right_handle == Some(movement.entity) {
                                    ...
                                    if left_handle == Some(movement.entity) && right_handle.is_some() {
                                        ...
                                        debug!("[NUDGE] Smooth constraint: left handle moved, adjusting right handle to ({:.1}, {:.1})", opposite_position.x, opposite_position.y);
                                        ...
                                        debug!("[NUDGE] Smooth constraint: right handle moved, adjusting left handle to ({:.1}, {:.1})", opposite_position.x, opposite_position.y);
                                    }
                                }
                            }
                        }
                ...
                // Apply smooth constraint adjustments to transforms
                if !smooth_adjustments.is_empty() {
                    for (entity, new_position) in &smooth_adjustments {
                        if let Ok((_, mut transform, _, _)) = queries.p2().get_mut(*entity) {
                            transform.translation.x = new_position.x;
                            transform.translation.y = new_position.y;
                            debug!(
                                "[NUDGE] Smooth constraint: Updated position for {:?} to ({:.1}, {:.1})",
                                entity, new_position.x, new_position.y
                            );
                        }
                    }

                    debug!("[NUDGE] Applied {} smooth curve constraint adjustments", smooth_adjustments.len());
                }
            }
