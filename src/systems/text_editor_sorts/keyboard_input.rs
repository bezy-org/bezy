...
pub fn handle_arabic_text_input(
    ...
    for ev in key_evr.read() {
        ...
        if let Some(ch) = arabic_char {
                ...
                if text_editor_state.cursor_position > 0 {
                    ...
                    for i in start..text_editor_state.cursor_position {
                        if let Some(sort) = text_editor_state.buffer.get(i) {
                            ...
                            if let crate::core::state::text_editor::buffer::SortKind::Glyph { glyph_name, .. } = &sort.kind {
                ...
                insert_arabic_glyph(&mut text_editor_state, &glyph_name, &_app_state, &_fontir_app_state);
...
fn insert_arabic_glyph(
    ...
    text_editor_state.buffer.insert(text_editor_state.cursor_position, sort);
...
fn get_glyph_advance_width(
    ...
    if let Some(app_state) = app_state.as_ref() {
        if let Some(glyph_data) = app_state.workspace.font.glyphs.get(glyph_name) {
            ...
            info!("📏 ADVANCE WIDTH: Glyph '{}' from AppState = {}", glyph_name, width);
            return width;
        }
        warn!("⚠️ ADVANCE WIDTH: Glyph '{}' not found in AppState", glyph_name);
    } else if let Some(fontir_state) = fontir_app_state.as_ref() {
        let width = fontir_state.get_glyph_advance_width(glyph_name);
        info!("📏 ADVANCE WIDTH: Glyph '{}' from FontIR = {}", glyph_name, width);
        return width;
    }

    // Fallback default width
    warn!("⚠️ ADVANCE WIDTH: Using fallback width 500.0 for glyph '{}'", glyph_name);
    500.0
}
